...
<div class="clientsbox center">
	...
	<?php }?>
	<?php if(OC_APP::isEnabled('thirstrunwizard')) {?>
	<p class="center"><a class="button" href="#" id="showWizard"><?php p($l->t('Show Thirst Run Wizard again'));?></a></p>
	<?php }?>
</div>
...
<form id="passwordform">
	<fieldset class="personalblock">
		...
		<input type="password" id="pass3" name="personal-password-confirm"
			placeholder="<?php echo $l->t('New password');?>"
			data-typetoggle="#personal-show" autocomplete="off" />
...
<form id="additionalEmail">
    <fieldset class="personalblock">

        <h2><?php p($l->t('Alert Email'));?></h2>

	   <!-- <p><em><?php p($l->t('Additional email address for notifications'));?></em></p>-->
	    <p><em><?php p($l->t('Additional email address for alerts'));?></em></p>
	    <table id="dataTable" class="form" border="1">
            <tbody>
            <?php foreach($_['notificationEmail'] as $email):?>
            <tr>
                <p>
                <td >
                ...
                <td>
                    <input name="notEmail[]" type="text" value="<?php p($email); ?>"
                           placeholder="<?php p($l->t('Your email address for alerts'));?>" /><br />
                </td>
                </p>
            </tr>
            <?php endforeach;?>
	    ...
	    <p>
		    <input id="addEmail" type="button" value="Add Another Email"/>
		    <input id="removeEmail" type="button" value="Remove Selected Email"/>
	    </p>

	    <p><em>Events to be notified using email</em></p>

<!--	    <p>noteTypes:--><?php //echo json_encode( $_['noteTypes'] ) ?><!--</p>-->
	    <?php
	        $noteTypes = $_['noteTypes'];
	        function genOption( $id, $typesArray )
	        {
		        $str =  '<input name="notification_opt[]"'
		                . ' value=' . $id
		                . ' type="checkbox"';
				if ( in_array( $id, $typesArray ) && ( $typesArray[0] != "" ) ) // second compare needed because PHP treats a null array as though it contains a zero
					$str .=  " checked='checked'";
		        $str .= " > ";
		        $str .= \OCA\MailNotify\NotificationTypes::$Codes[$id] . " - " . \OCA\MailNotify\NotificationTypes::$CodeExplanations[$id] . "<br />";
		        echo $str."\n";
	        }
	    ?>
		<?php foreach( \OCA\MailNotify\NotificationTypes::$CodesArray as $i )
	        genOption( $i, $noteTypes  );
	    ?>

	    <p>
            <!--<input id="saveEmail" type="button" value="Save Notification Settings"/><span class="msg"></span>-->
            <input id="saveEmail" type="button" value="Save Alert Settings"/><span class="msg"></span>
        </p>
    </fieldset>
</form>
...
<fieldset class="personalblock">
	...
	<code><?php print_unescaped('http://'.getenv('SERVER_NAME').str_replace('index.php', '', getenv('SCRIPT_NAME')).'remote.php/webdav/'); ?></code><br />
