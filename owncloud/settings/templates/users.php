...
<div id="controls">
	<form id="newuser" autocomplete="off">
		...
		<input type="password" id="newuserpassword" placeholder="<?php p($l->t('Password'))?>" /> 
		<select	class="groupsselect" id="newusergroups" data-placeholder="groups" 
			title="<?php p($l->t('Jurisdiction'))?>" multiple="multiple">
			<?php foreach($_["groups"] as $group): ?>
			  <option <?php if(count($_["groups"]) <= 1) { p('selected'); }?> value="<?php p($group['name']);?>"><?php p($group['name']);?></option>
			<?php endforeach;?>
		</select>
<!--		<input id="email" type="text" placeholder="<?php // p($l->('email'))?>" />-->
		...
		<span><?php p($l->t('Default Storage'));?></span>
			...
			<?php if((bool) !$_['isadmin']): ?>
				<select class='quota' disabled="disabled">
					<option selected="selected">
					<?php if($_['default_quota'] === 'none')  p($l->t('Unlimited'));?>
					</option>
				</select>
			<?php endif; ?>
	</div>
...
<table class="hascontrols grid" data-groups="<?php p(json_encode($allGroups));?>">
	<thead>
		<tr>
			...
			<th id="headerEmail"><?php p($l->t( 'Email' )); ?></th>
			<th id="headerPassword"><?php p($l->t( 'Password' )); ?></th>
			<?php if((bool) $_['isadmin']): ?>
			<th id="headerFailedLogins">Failed Logins</th>
			<th id="headerLockStatus">Lock Status</th>
			<th id="headerGroups"><?php p($l->t( 'Jurisdictions' )); ?></th>
			<?php endif;?>
			<?php if(is_array($_['subadmins']) || $_['subadmins']): ?>
			<th id="headerSubAdmins"><?php p($l->t('Jurisdiction Admin')); ?></th>
			<?php endif;?>
			<th id="headerQuota"><?php if((bool) $_['isadmin']) p($l->t('Storage')); ?></th>
	...
	<tbody>
		...
		<tr data-uid="<?php p($user["name"]) ?>"
			...
			<td id="editemail" class="email"><span><?php p($user["email"]); ?></span> <img class="svg action"
				src="<?php p(image_path('core', 'actions/rename.svg'))?>"
				alt="<?php p($l->t("change email"))?>" title="<?php p($l->t("change email"))?>"/>
			</td>
			...
			<td class="failedLogins">
				<?php p($user['error_count']); ?>
			</td>
			<td class="lockStatus">
				<?php p($user['locked']); ?>
				<a href="#" id='lock_toggle' class="action lock" original-title="<?php !$user['locked'] ? p("Lock") : p("Unlock")?>">
					<img src="<?php print_unescaped(image_path('core', 'actions/lock.svg'))?>" class="svg"/>
				</a>
			</td>
			...
			<td class="groups">
				...
				data-placeholder="groups" title="<?php p($l->t('Jurisdiction'))?>"
				multiple="multiple">
					<?php foreach($_["groups"] as $group): ?>
			...
			<?php if(is_array($_['subadmins']) || $_['subadmins']): ?>
			  <td class="subadmins"><select
				...
				data-placeholder="subadmins" title="<?php p($l->t('Select One'))?>"
				multiple="multiple">
				<?php $_['subadmingroups'] = $user['groups']; ?>
			        	<?php foreach($_["subadmingroups"] as $group): ?>
					    <option value="<?php p($group);?>"><?php p($group);?></option>
					<?php endforeach;?>
					<?php if((count ($_['subadmingroups']))== 0) : ?>
			  		    <option id="freshuser" value="ERROR">ERROR</option> 
					<?php endif;?>
