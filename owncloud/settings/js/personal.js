...
function changeNotificationAddress(){

    //Gets the form to be posted
    var postRaw = $( "#additionalEmail" );
    var post = postRaw.serialize();
    OC.msg.startSaving('#additionalEmail .msg');


    $.post( 'ajax/notificationaddress.php', post, function(data){
    OC.msg.finishedSaving('#additionalEmail .msg', data);
    });
...
/** MITRE Code Starts */
    $("#addEmail").click( function(){
        ...
        if(rowCount < 5){                            // limit the user from creating fields more than your limits
            ...
            newcell2.innerHTML = '<input name="notEmail[]" type="text" value="">';
        }else{
            alert("Maximum Emails per user is 5");
        ...
        for(var i=0; i<rowCount; i++) {
            ...
            if(null != chkbox && true == chkbox.nextElementSibling.checked) {
                if(rowCount <= 1) {               // limit the user from removing all the fields
                    alert("Cannot Remove all Email Addresses.");
                    break;
                }
                table.deleteRow(i);
                rowCount--;
    ...
    $('#saveEmail').click(function(){
        var emailForm = document.forms.additionalEmail;
        var myEmails = emailForm.elements['notEmail[]'];
        var emails = document.getElementsByClassName("notEmail");
                 if (myEmails != null ){
                          if(typeof timeout !== 'undefined'){
        	                  clearTimeout(timeout);
                          }
        	          timeout = setTimeout('changeNotificationAddress()',1000);
       		  }
        });
/** MITRE Code Ends */
	...
	$("#passwordbutton").click( function(){
		...
		//We test for password strength in the owncloud core now. (plus this is wrong).
		/*if ($('#pass2').val().match("^(?=.*\d+)(?=.*[a-zA-Z])[0-9a-zA-Z!@#$%]{6,10}$") == null){
			$('#passwordchanged').hide();
			$('#passworderror').show();
			return false;
		}*/
