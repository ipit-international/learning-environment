...
function toggle_lock(uid) {
 $.post(OC.filePath('settings', 'ajax', 'togglelock.php'),
  {username:uid},
  function (r) { 
	if (r.status=='success'){
	  var row = $("tr[data-uid|='"+uid+"']");

	  var lf; (r.data.action=='Lock') ? lf = 1 : lf = 0;
	  
          var lock_img = $('<img class="svg action">').attr({src: OC.imagePath('core', 'actions/lock')});
	  var lock_link = $('<a class="action lock">').attr({ href: '#', 'original-title': r.data.action}).append(lock_img);
	 
	  $(".tipsy").remove();
	   
          row.find("td.lockStatus").html(lock_link).prepend(lf+" ");
          (r.data.action=="Unlock") ? row.find("td.failedLogins").text(0) : lf = lf;
	} else {
...
var UserList = {
	...
	do_delete: function (uid) {
		...
		OC.Notification.showHtml(t('users', 'deleted') + ' ' + escapeHTML(uid) + '<span class="undo">' + t('users', 'undo') + '</span> or <span class="confirm">confirm</span>');
	},
	...
	add: function (username, displayname, email, groups, subadmin, quota, sort) {
		...
		//tr.find('td.email > span').text(email);
		...
		$.get(OC.Router.generate('settings_ajax_userlist', { offset: UserList.offset, limit: UserList.usersToLoad }), function (result) {
			if (result.status === 'success') {
				...
				$.each(result.data, function (index, user) {
					...
					var tr = UserList.add(user.name, user.displayname, user.email, user.groups, user.subadmin, user.quota, false);
					if (index === 9) {
	...
	applyMultiplySelect: function (element) {
		...
		if ($(element).attr('class') === 'groupsselect') {
			...
			// MITRE modification begins:  the block below is commented out to prevent new groups from being made on the fly since we now need to have
			//                      more data for groups
			var label = null;
//			if (isadmin) {
...
$(document).ready(function () {
	...
	$('table').on('click', 'td.lockStatus>a', function (event) {
		var row = $(this).parent().parent();
		var uid = $(row).attr('data-uid');
		// Call function for handling delete/undo
		toggle_lock(uid);
	...
	$('table').on('click', 'td.email>img', function (event) {
		event.stopPropagation();
		var img = $(this);
		var uid = img.parent().parent().attr('data-uid');
		var email = escapeHTML(img.parent().parent().attr('data-email'));
		var input = $('<input type="text" value="' + email + '">');
		img.css('display', 'none');
		img.parent().children('span').replaceWith(input);
		input.focus();
		input.keypress(function (event) {
			if (event.keyCode === 13) {
				if ($(this).val().length > 0) {
					$.post(
						OC.filePath('settings', 'ajax', 'changeemail.php'),
						{username: uid, email: $(this).val()},
						function (result) {
						  if (result.status!=='success'){
						    OC.dialogs.alert(
						      t('settings', 'A valid email must be provided.'), 
						      t('settings', 'Error changing user'));
						    return false;
						  }
						}
					);
	...
	$('table').on('click', 'td.email', function (event) {
		$(this).children('img').click();
	});
	...
	$('#newuser').submit(function (event) {
		...
		//Password strength is now checked deep in the owncloud core, making a check here pointless.
		/*if ($.trim(password).match("^(?=.*\d+)(?=.*[a-zA-Z])[0-9a-zA-Z!@#$%]{6,10}$") === null) {
			OC.dialogs.alert(
        ...
        //newly created users must be put into a group
        if(groups.length == 0) {
            OC.dialogs.alert(
                t('settings', 'A user must belong to a juristdiction'),
                t('settings', 'Error creating user'));
            return false;
        }
			...
			function (result) {
				if (result.status !== 'success') {
					...
					if($('tr[data-uid="' + username + '"]').length === 0) {
						UserList.add(username, username, email, result.data.groups, null, 'default', true);
