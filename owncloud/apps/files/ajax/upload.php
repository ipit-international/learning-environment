<?php
//START MITRE CODE
$signal_upload_success = 'upload_success';
$signal_upload_failure = 'upload_failure';
$h_data = array('reason' => 'none');
$h_data = array_merge($h_data, array('path'=>'/'));//todo change/check/review this
//END MITRE CODE
...
if (empty($_POST['dirToken'])) {
	...
	\OCP\Util::writeLog('upload.php', "RPG: $dir",4);

	if (!$dir || empty($dir) || $dir === false) {

        	//START MITRE CODE
        	$h_data = array_merge($h_data, array('reason'=>'Unable to set upload directory.'));
        	OCP\Util::emitHook(\OC\Files\Filesystem::CLASSNAME,$signal_upload_failure, $h_data);
        	//END MITRE CODE	
	...
	if ($linkItem === false) {
		...
		//START MITRE CODE
	        $h_data = array_merge($h_data, array('reason'=>'Invalid Token.'));
        	OCP\Util::emitHook(\OC\Files\Filesystem::CLASSNAME,$signal_upload_failure, $h_data);
	        //END MITRE CODE
...
if (!isset($_FILES['files'])) {
	...
	//START MITRE CODE
	$h_data = array_merge($h_data, array('reason'=>'No file was uploaded. Unknown error.'));
    	OCP\Util::emitHook(\OC\Files\Filesystem::CLASSNAME,$signal_upload_failure, $h_data);
    	//END MITRE CODE
...
foreach ($_FILES['files']['error'] as $error) {
	if ($error != 0) {
		...
		//START MITRE CODE
	        $h_data = array_merge($h_data, array('reason'=>$errors[$error]));
        	OCP\Util::emitHook(\OC\Files\Filesystem::CLASSNAME,$signal_upload_failure, $h_data);
        	//END MITRE CODE
...
if ($maxUploadFileSize >= 0 and $totalSize > $maxUploadFileSize) {
	...
	//START MITRE CODE
    	$h_data = array_merge($h_data, array('reason'=>'Not enough storage available'));
    	OCP\Util::emitHook(\OC\Files\Filesystem::CLASSNAME,$signal_upload_failure, $h_data);
	//END MITRE CODE
...
if (strpos($dir, '..') === false) {
	...
	for ($i = 0; $i < $fileCount; $i++) {
		...
		if (isset($_POST['resolution']) && $_POST['resolution']==='autorename') {
			...
			\OCP\Util::writeLog('upload.php',"Path in upload.php: ".$target, \OCP\Util::FATAL);
		}
...
if ($error === false) {
	...
	//START MITRE CODE
    	$h_data = array_merge($h_data, array('path'=>$target));
    	OCP\Util::emitHook(\OC\Files\Filesystem::CLASSNAME,$signal_upload_success, $h_data);
    	//END MITRE CODE
	...
	//START MITRE CODE
    	$h_data = array_merge($h_data, array('reason'=>$error));
    	OCP\Util::emitHook(\OC\Files\Filesystem::CLASSNAME,$signal_upload_failure, $h_data);
    	//END MITRE CODE
