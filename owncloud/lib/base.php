...
class OC {
	...
	*/
	...
	public static function initSession() {
		...
		//MITRE: set a session lifetime that is actually effective
		$cookieLifeTime = self::getLoginCookieLifeTime();
	 ...
	 * @return int
	 */
	private static function getLoginCookieLifeTime() {
                return OC_Config::getValue('cookie_lifetime', 60 * 60 * 24);
        }
	...
	protected static function handleLogin() {
		...
		//MITRE Modification 
		$values = array();
		$values = OC::tryFormLogin();
	        	
		if($values['locked']) {
			$error[] = 'accountlocked';
			if ( OC_Config::getValue('log_authfailip', false) ) {
				OC_Log::write('core', 'Login failed: user \''.$_POST["user"].'\' , account locked, IP:'.$_SERVER['REMOTE_ADDR'],
				OC_Log::WARN);
			} else {
				OC_Log::write('core', 'Login failed: user \''.$_POST["user"].'\' , account locked, IP:set log_authfailip=true in conf',
                        	OC_Log::WARN);
			}
		}
		...
		if($values['success']) {
			...
			if ( OC_Config::getValue('log_authfailip', false) ) {
				OC_Log::write('core', 'Login failed: user \''.$_POST["user"].'\' , invalid password, IP:'.$_SERVER['REMOTE_ADDR'],
				OC_Log::WARN);
			} else {
				OC_Log::write('core', 'Login failed: user \''.$_POST["user"].'\' , invalid password, IP:set log_authfailip=true in conf',
                        	OC_Log::WARN);
			}
		}
		//MITRE Modification End
	...
	protected static function tryFormLogin() {
		$output = array('success' => false, 'locked' => false);
		...
		if(!OC_User::userExists($_POST["user"])) 
			return $output;
		//If the user is locked out, go no further.
		if(OC_User::getLocked($_POST["user"])) {
			$output['locked'] = true;
			$output['success'] = true;
			return $output;
		}
		...
		if (OC_User::login($_POST["user"], $_POST["password"])) {
			...
			//successfully logged in so reset the error count field to zero
			//OC_User::setErrorCount($userid,0);
			
			OC_Util::redirectToDefaultPage();
			exit();
		}
		
		//failed the login, so need to increment the error count by 1.
		//SEPT 11TH EDIT: MOVED THIS LOGIC INTO THE ACTUAL LOGIN FUNCTION OF SESSION.PHP
		//$ec = OC_User::getErrorCount($_POST["user"]) + 1;
		//OC_User::setErrorCount($_POST["user"], $ec);

		//$maxNumberLoginAttempts = OC_Config::getValue('max_login_attempts', 5);
		//if(OC_User::getErrorCount($_POST["user"]) >= $maxNumberLoginAttempts) {
		//	OC_User::setLocked($_POST["user"], true);
		//}
