		...
		if (is_resource($data)) { //not having to deal with streams in file_put_contents makes life easier
			...
			if (\OC_FileProxy::runPreProxies('file_put_contents', $absolutePath, $data)
				...
				if ($this->shouldEmitHooks($path)) {
					if (!$exists) {
						\OCP\Util::writeLog("lib-private-files-view", "Emitting Filesystem.create with path: ".$path, 4);
						\OC_Hook::emit(
					...
					\OCP\Util::writeLog("lib-private-files-view", "Emitting Filesystem.write emit on line 297 with path: ".$path, 4);
					\OC_Hook::emit(
						Filesystem::CLASSNAME,
		...
		if (
			...
			if ($this->shouldEmitHooks() && (Cache\Scanner::isPartialFile($path1) && !Cache\Scanner::isPartialFile($path2))) {
				// if it was a rename from a part file to a regular file it was a write and not a rename operation
					\OCP\Util::writeLog("lib-private-files-view", "Emitting Filesystem.write emit on line 382 with path: ".$path, 4);
				\OC_Hook::emit(
	...
	public function copy($path1, $path2) {
		...
		if (
			...
			if ($this->shouldEmitHooks()) {
				...
				if ($run) {
					\OCP\Util::writeLog("lib-private-files-view", "Emitting Filesystem.write emit on line 503 with path: ".$path, 4);
					\OC_Hook::emit(
						Filesystem::CLASSNAME,
						Filesystem::signal_write,
						array(
