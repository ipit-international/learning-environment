...
class OC_User {
	...
	public static function deleteUser($uid) {
		...
		if ($user) {
			...
			if ($result) {
				...
				$sql = \OC_DB::prepare('DELETE FROM `*PREFIX*permissions` WHERE `user` = ? ');
				$sr = $sql->execute(array($uid));
	...
	//MITRE CODE
	public static function getErrorCount($uid = null) {
		if ($uid) {
			$user = self::getManager()->get($uid);
			if ($user) {
				return $user->getErrorCount();
			} else {
				return $uid;
			}
		} else {
	...
	public static function getLocked($uid = null) {
		if ($uid) {
			$user = self::getManager()->get($uid);
			if ($user) {
	...
	public static function setErrorCount($uid, $error_count = 0) {
		$user = self::getManager()->get($uid);
		if ($user) {
			return $user->setErrorCount($error_count);
		} else {
			return false;
		}
	}
	public static function setLocked($uid, $locked = false) {
		$user = self::getManager()->get($uid);
		if ($user) {
			return $user->setLocked($locked);
		} else {
			return false;
		}
	}
	...
	public static function setPassword($uid, $password, $recoveryPassword = null) {
		if (empty($password)) return false;//Dump with 

		//preg_match returns FALSE in the case of an error, 0 in the case where a match is unfound.
	        $pattern = '/.{6,}/';//6 or more characters
		if (!preg_match($pattern,$password)) {
			//OC_Log::write('SET_PASSWORD', 'Insufficient length (6+) password: '.$password . '.', OC_Log::FATAL);
			return false;
		}

		$pattern = '/[0-9]/';//at least 1 number
	        if (!preg_match($pattern,$password)) { 
			//OC_Log::write('SET_PASSWORD', 'Failed to find digit in password: '.$password . '.', OC_Log::FATAL);
			return false;
		}

		$pattern = '/[A-Z]/';//at least 1 upper case character
	        if (!preg_match($pattern,$password)) { 
			//OC_Log::write('SET_PASSWORD', 'Failed to find upper case character in password: '.$password . '.', OC_Log::FATAL);
			return false;
		}

		$pattern = '/[a-z]/';//at least 1 lower case character
	        if (!preg_match($pattern,$password)) { 
			//OC_Log::write('SET_PASSWORD', 'Failed to find lower case character in password: '.$password . '.', OC_Log::FATAL);
			return false;
		}

		$pattern = '/[~`!@#$%^&\*\(\)_\-+=\}\{\]\[|\";:\/\?\.\<\,\>\']/';//at least 1 special character (minus ')
	        if (!preg_match($pattern,$password)) {
			//OC_Log::write('SET_PASSWORD', 'Failed to find special character in password: '.$password . '.', OC_Log::FATAL);
			return false;
		}
		$user = self::getManager()->get($uid);
		if ($user) {
			//OC_Log::write('SET_PASSWORD', 'Successfully set the password.', OC_Log::FATAL);
