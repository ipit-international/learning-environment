...
class Session implements Emitter, \OCP\IUserSession {
	...
	public function login($uid, $password) {
		...
		if($user !== false) {
			...
			if (!is_null($user) && !\OC_User::getLocked($uid)) {
				if ($user->isEnabled()) {
					$this->setUser($user);
					$this->setLoginName($uid);
					$this->manager->emit('\OC\User', 'postLogin', array($user, $password));
					\OC_User::setErrorCount($uid,0);
					return true;
				} else {
					return false;
				}
			...
			$ec = \OC_User::getErrorCount($uid) + 1;
			\OC_User::setErrorCount($uid, $ec);
			$maxNumberLoginAttempts = \OC_Config::getValue('max_login_attempts', 5);
	                if(\OC_User::getErrorCount($uid) >= $maxNumberLoginAttempts) {
        	                \OC_User::setLocked($uid, true);
                	}
			return false;
		}
	}
