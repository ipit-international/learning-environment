...
class OC_User_Database extends OC_User_Backend {
	...
	//MITRE CODE
	public function getErrorCount($uid) {
		$query = OC_DB::prepare('SELECT `error_count` FROM `*PREFIX*users` WHERE `uid` = ?');
		$result = $query->execute(array($uid))->fetchAll();
	...
	public function getLocked($uid) {
		$query = OC_DB::prepare('SELECT `locked` FROM `*PREFIX*users` WHERE `uid` = ?');
		$result = $query->execute(array($uid))->fetchAll();
	...
	public function setErrorCount($uid, $error_count) {
		\OC_Log::write('database', "attempting to set $uid's error_count to $error_count as requested.", \OC_Log::WARN);
		if ($this->userExists($uid)) {
			$query = OC_DB::prepare('UPDATE `*PREFIX*users` SET `error_count` = ? WHERE LOWER(`uid`) = ?');
			$query->execute(array($error_count, $uid));
			return true;
		} else {
			return false;
		}
	}
	public function setLocked($uid, $locked) {
		if ($this->userExists($uid)) {
			$query = OC_DB::prepare('UPDATE `*PREFIX*users` SET `locked` = ? WHERE LOWER(`uid`) = ?');
			$query->execute(array($locked, $uid));
			return true;
		} else {
			return false;
		}
