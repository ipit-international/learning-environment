...
// 				}
                ...
                //get the IGAs for this user
                \OCP\Util::writeLog('share',"TDM: calling MC_User::getIGAPartners...",4);
                $igaCountries = \OCP\MC_User::getIGAPartners();
                $groups = array_intersect($groups,$igaCountries);
				...
				while ($count < 15 && count($users) == $limit) {
					...
					if ($sharePolicy == 'groups_only') {
						...
						// MITRE modification:  this prevents users from showing up in type-ahead in GUI,
						//                      since we want sharing to only happen between groups
//						$users = OC_User::getDisplayNames($_GET['search'], $limit, $offset);
                ...
                //foreach($igaCountries as $igaCountry) {
                        //\OCP\Util::writeLog('share',"TDM: igaCountry = $igaCountry",4);
						if ($count < 15) {
						if (!isset($_GET['itemShares'])
